...
def set_transforms(model_config: ModelConfig, file_manager: FileManager):
    ...
    if model_config.add_transforms:
        ...
        if model_config.test_time_augmentation:
            ...
            test_transforms = set_test_transforms(file_manager.logger, img_size, normalize)
            return train_transforms, val_transforms, test_transforms
        
        file_manager.logger.info(f"Train augmentations: \n {train_transforms}")
        file_manager.logger.info(f"Validation/Test augmentations: \n {val_transforms}")
        ...
        if model_config.test_time_augmentation:
            file_manager.logger.info(f"Train augmentations: \n {train_transforms}")
            file_manager.logger.info(f"Validation augmentations: \n {val_transforms}")
            test_transforms = set_test_transforms(img_size, normalize)
            return train_transforms, val_transforms, test_transforms
        file_manager.logger.info(f"Train augmentations: \n {train_transforms}")
        file_manager.logger.info(f"Validation/Test augmentations: \n {val_transforms}")
        return train_transforms, val_transforms, val_transforms
