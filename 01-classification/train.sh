...
for MODEL in "${MODELS[@]}"; do
    for TASK in "${TASKS[@]}"; do
        ...
        sbatch --partition=$PARTITION \
            ...
            --job-name=$JOB_NAME \
            --output=$OUTPUT_FILE \
            --export=TEST_MODE=$TEST_MODE,OUTPUT_FOLDER=$OUTPUT_FOLDER,CODE_PATH=$CODE_PATH,IDUN_TIME=$IDUN_TIME,MODEL=$MODEL,BATCH_SIZE=$BATCH_SIZE,LEARNING_RATE=$LEARNING_RATE,NUM_EPOCHS=$NUM_EPOCHS,TASK=$TASK,LOSS=$LOSS \
